...
export function OwnedObjects({ refreshKey, setRefreshKey }: RefreshProps) {
  ...
  // State'ler - her hero için ayrı değerler tutmak için object kullanıyoruz
  ...
  // Kullanıcının sahip olduğu Hero nesnelerini sorgula
  const { data, isPending, error } = useSuiClientQuery(
    "getOwnedObjects",
    {
      owner: account?.address as string,
      filter: {
        StructType: `${packageId}::hero::Hero`, // Sadece Hero tipindeki nesneleri filtrele
      },
      options: {
        showContent: true, // Nesne içeriğini göster
        showType: true, // Nesne tipini göster
      },
    },
    {
      enabled: !!account && !!packageId, // Sadece hesap ve packageId varsa sorgula
